...
REQUIRED_COLUMNS = ["sample", "fastq_1", "fastq_2", "read_layout", "host_ref"]
...
def validate_rows(rows, platform):
    ...
    if not rows:
        errors.append("samplesheet contains no samples")
        return errors
...
def main():
    parser = argparse.ArgumentParser(description="Validate a HOMES metagenomics samplesheet.")
            ...
            writer.writerow(
                {
                    ...
                    "read_layout": clean(row.get("read_layout")).lower(),
                    "host_ref": resolve_path(row.get("host_ref"), base_dir),
                }
            )

    print(f"Validated {len(rows)} {args.platform} metagenomics sample(s).", file=sys.stderr)
