//
// Subworkflow with utility functions specific to the HOMES workflow template
//
...
def checkProfileProvided(nextflow_cli_args) {
    ...
    if (nextflow_cli_args[0]) {
        log.warn(
            "HOMES workflows do not accept positional arguments. The positional argument `${nextflow_cli_args[0]}` has been detected.\n" + "HINT: A common mistake is to provide multiple values separated by spaces e.g. `-profile test, docker`.\n"
        )
    }
}
