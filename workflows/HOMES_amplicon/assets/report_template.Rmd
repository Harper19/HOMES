...
params:
    ...
    vsearch_lca_ref_tax_title: FALSE
    vsearch_lca_ref_tax_file: ""
    vsearch_lca_ref_tax_citation: ""
    exclude_taxa: ""
    min_frequency: ""
    min_samples: ""
    qiime2_filtertaxa: ""
...
report_subtitle <- paste0('HOMES_amplicon ', params$platform_label, ' workflow ', ampliseq_version)
```
...
The bioinformatics analysis pipeline [HOMES_amplicon](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon) is used for amplicon sequencing,
supporting denoising of common marker-gene amplicons and a variety of taxonomic databases for taxonomic assignment including 16S, 18S and ITS.
    "))
}
```
...
if ( params$flag_single_end ) {
    ...
    cat("Processing stages are: input - read pairs into DADA2, filtered - read pairs passed quality filtering, ",
        ...
        "denoisedPairs - read pairs considered for merging, ",
...
if ( params$flag_single_end ) {
    ...
    steps_t <- c(rep("excluded by filtering", n_samples), rep("not denoised", n_samples),
                rep("suspected bimera", n_samples), rep("reads in final ASVs", n_samples))
    ...
    dada_stats_ex <- data.frame(sample = dada_stats$sample,
                            ...
                            denoisedPairs = (dada_stats$denoisedF+dada_stats$denoisedR)/2-dada_stats$denoisedPairs,
                            merged = dada_stats$denoisedPairs-dada_stats$merged,
    ...
    steps_t <- c(rep("excluded by filtering", n_samples),
                rep("forward not denoised", n_samples),
                rep("reverse not denoised", n_samples),
                rep("one read not denoised", n_samples),
                rep("not merged", n_samples),
                rep("suspected bimera", n_samples),
                rep("reads in final ASVs", n_samples))
    # stack the column for percentage of asvs
...
```{r, eval = !isFALSE(params$asv_table_path), results='asis'}
cat("# Abundance\n\n")
cat("## ASV abundance overview\n\n")

asv_abund <- read.table(file = params$asv_table_path, header = TRUE, sep = "\t", check.names = FALSE)
sample_cols <- setdiff(colnames(asv_abund), "ASV_ID")
asv_abund[sample_cols] <- lapply(asv_abund[sample_cols], function(x) as.numeric(as.character(x)))

sample_totals <- data.frame(
    Sample = sample_cols,
    Total_ASV_counts = colSums(asv_abund[sample_cols], na.rm = TRUE),
    check.names = FALSE
)
sample_totals$Relative_depth_percent <- round(sample_totals$Total_ASV_counts / sum(sample_totals$Total_ASV_counts) * 100, 2)

cat("The table below summarizes final ASV read counts per sample after DADA2 processing and configured ASV filtering.\n\n")
print(knitr::kable(sample_totals, format = "html", row.names = FALSE))

asv_abund$Total_count <- rowSums(asv_abund[sample_cols], na.rm = TRUE)
asv_abund$Relative_abundance_percent <- round(asv_abund$Total_count / sum(asv_abund$Total_count) * 100, 4)
top_asvs <- head(asv_abund[order(-asv_abund$Total_count), c("ASV_ID", "Total_count", "Relative_abundance_percent")], 10)

cat("\n\nThe top ASVs by total abundance across all samples are shown below.\n\n")
print(knitr::kable(top_asvs, format = "html", row.names = FALSE))
```

```{r, eval = !isFALSE(params$asv_table_path) && !isFALSE(params$dada2_taxonomy), results='asis'}
cat("## Taxonomic relative abundance\n\n")

asv_tax_abund <- read.table(params$dada2_taxonomy, header = TRUE, sep = "\t", check.names = FALSE)
tax_ranks <- setdiff(colnames(asv_tax_abund), c("ASV_ID", "confidence", "sequence"))
classified_ranks <- tax_ranks[colSums(asv_tax_abund[tax_ranks] != "" & !is.na(asv_tax_abund[tax_ranks])) > 0]

if (length(classified_ranks) > 0) {
    requested_levels <- trimws(unlist(strsplit(params$tax_abundance_levels, ",")))
    requested_levels <- requested_levels[requested_levels != ""]
    if (length(requested_levels) == 0 || tolower(requested_levels[1]) == "all") {
        abundance_ranks <- classified_ranks
        cat("DADA2 taxonomy was combined with ASV counts to summarize relative abundance at all classified taxonomy ranks: `",
            paste(abundance_ranks, collapse = "`, `"), "`.\n\n", sep = "")
    } else {
        abundance_ranks <- classified_ranks[tolower(classified_ranks) %in% tolower(requested_levels)]
        cat("DADA2 taxonomy was combined with ASV counts to summarize relative abundance at requested taxonomy ranks: `",
            paste(requested_levels, collapse = "`, `"), "`.\n\n", sep = "")
    }

    if (length(abundance_ranks) == 0) {
        cat("None of the requested taxonomy ranks were available in this run. Available ranks were: `",
            paste(classified_ranks, collapse = "`, `"), "`.\n\n", sep = "")
    }

    if (length(abundance_ranks) > 0) {
        level_ids <- paste0("illumina-abundance-", tolower(gsub("[^A-Za-z0-9]+", "-", abundance_ranks)))
        cat('<p>Relative abundance is calculated independently within each sample. Rows are taxa and columns are sample IDs.</p>\n')
        cat('<label for="illumina-abundance-level-select"><strong>Taxonomy level</strong></label> ')
        cat('<select id="illumina-abundance-level-select" onchange="showIlluminaAbundanceLevel()">\n')
        for (i in seq_along(abundance_ranks)) {
            cat('<option value="', level_ids[i], '">', abundance_ranks[i], '</option>\n', sep = "")
        }
        cat('</select>\n')
    }

    for (abundance_rank in abundance_ranks) {
        tax_counts <- merge(asv_abund[, c("ASV_ID", sample_cols)], asv_tax_abund[, c("ASV_ID", abundance_rank)], by = "ASV_ID", all.x = TRUE)
        tax_counts$Taxon <- tax_counts[[abundance_rank]]
        tax_counts$Taxon[is.na(tax_counts$Taxon) | trimws(tax_counts$Taxon) == ""] <- "Unclassified"
        tax_counts$Taxon <- paste0(abundance_rank, ": ", tax_counts$Taxon)

        tax_summary <- aggregate(tax_counts[sample_cols], by = list(Taxon = tax_counts$Taxon), FUN = sum, na.rm = TRUE)
        sample_depth <- colSums(asv_abund[sample_cols], na.rm = TRUE)
        tax_long <- reshape(
            tax_summary,
            varying = sample_cols,
            v.names = "Count",
        ...
        tax_long$Sample <- as.character(tax_long$Sample)
        tax_long$Count <- as.numeric(tax_long$Count)
        tax_long$Sample_total_count <- sample_depth[tax_long$Sample]
        tax_long$Relative_abundance_percent <- ifelse(
            tax_long$Sample_total_count > 0,
            round(tax_long$Count / tax_long$Sample_total_count * 100, 4),
            0
        )
        tax_long <- tax_long[tax_long$Count > 0, c("Sample", "Taxon", "Count", "Sample_total_count", "Relative_abundance_percent")]
        tax_long <- tax_long[order(tax_long$Sample, -tax_long$Relative_abundance_percent, tax_long$Taxon), ]

        rank_file <- paste0("dada2_relative_abundance_", tolower(abundance_rank), "_per_sample.tsv")
        write.table(tax_long, file = rank_file, sep = "\t", quote = FALSE, row.names = FALSE)

        level_id <- paste0("illumina-abundance-", tolower(gsub("[^A-Za-z0-9]+", "-", abundance_rank)))
        display_style <- ifelse(abundance_rank == abundance_ranks[1], "", ' style="display:none"')
        cat('<div class="illumina-abundance-level" id="', level_id, '"', display_style, '>\n', sep = "")
        cat("<h3>", abundance_rank, "</h3>\n\n", sep = "")
        cat("The full per-sample long-format table is written to `", rank_file, "`.\n\n", sep = "")

        if (nrow(tax_long) == 0) {
            cat("No non-zero classified counts were available at this rank.\n\n")
            cat("</div>\n")
            next
        }

        matrix_df <- tax_long[, c("Taxon", "Sample", "Relative_abundance_percent")]
        abundance_matrix <- reshape(
            matrix_df,
            idvar = "Taxon",
            timevar = "Sample",
            direction = "wide"
        )
        colnames(abundance_matrix) <- sub("^Relative_abundance_percent\\.", "", colnames(abundance_matrix))
        abundance_matrix[is.na(abundance_matrix)] <- 0
        abundance_matrix <- abundance_matrix[order(abundance_matrix$Taxon), ]
        colnames(abundance_matrix)[colnames(abundance_matrix) == "Taxon"] <- "Taxa"
        print(knitr::kable(abundance_matrix, format = "html", row.names = FALSE, digits = 4))

        top_taxa_by_sample <- do.call(rbind, lapply(split(tax_long, tax_long$Sample), function(sample_df) {
            head(sample_df[order(-sample_df$Relative_abundance_percent, sample_df$Taxon), ], 10)
        }))
        top_taxa_names <- unique(top_taxa_by_sample$Taxon)
        plot_df <- tax_long
        plot_df$Plot_taxon <- ifelse(plot_df$Taxon %in% top_taxa_names, plot_df$Taxon, "Other")
        plot_df <- aggregate(
            cbind(Count, Relative_abundance_percent) ~ Sample + Plot_taxon,
            data = plot_df,
            FUN = sum
        )
        plot_df$Plot_taxon <- factor(plot_df$Plot_taxon, levels = c(sort(setdiff(unique(plot_df$Plot_taxon), "Other")), "Other"))

        plot_tax_abund <- ggplot(plot_df, aes(x = Sample, y = Relative_abundance_percent, fill = Plot_taxon)) +
            geom_bar(stat = "identity", width = 0.75) +
            ylab("Relative abundance within sample (%)") +
            xlab("Sample") +
            ggtitle(paste("Per-sample top taxa at", abundance_rank, "level")) +
            theme_bw() +
            theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank())

        svg_file <- paste0("dada2_relative_abundance_top_taxa_", tolower(abundance_rank), ".svg")
        svg(svg_file)
        print(plot_tax_abund)
        invisible(dev.off())
        cat('<p><img src="', svg_file, '" width="100%" style="float: none;"/></p>\n', sep = "")
        cat("</div>\n")
    }
    ...
    if (length(abundance_ranks) > 0) {
        cat('
<script>
function showIlluminaAbundanceLevel() {
  const selected = document.getElementById("illumina-abundance-level-select").value;
  document.querySelectorAll(".illumina-abundance-level").forEach(el => {
...
')
    ...
    cat("No classified taxonomy rank was available for taxonomic abundance summarization.\n\n")
}
```
...
any_taxonomy <- !isFALSE(params$dada2_taxonomy)  || !isFALSE(params$kraken2_taxonomy) || !isFALSE(params$qiime2_taxonomy) || !isFALSE(params$sintax_taxonomy) || !isFALSE(params$vsearch_lca_taxonomy) || !isFALSE(params$pplace_taxonomy)
```
...
<!-- Subsection on VSEARCH LCA taxonomy results -->

```{r, eval = !isFALSE(params$vsearch_lca_taxonomy), results='asis'}
# Header
cat("## VSEARCH LCA\n")

cat("The taxonomic classification was performed by [VSEARCH](https://github.com/torognes/vsearch)
...
# Build output string
outputstr <- "VSEARCH LCA classified "
for (row in seq_len(nrow(asv_classi_df))) {
    outputstr <- paste0(outputstr, asv_classi_df[row, ]$p_asv_classified,
        " % ASVs at ", asv_classi_df[row, ]$level, " level, ")
}
...
svg("vsearch_lca_taxonomic_classification_per_taxonomy_level.svg")
plot_asv_classi_df
invisible(dev.off())

cat("\n\nVSEARCH LCA taxonomy assignments can be found in folder [vsearch_lca](../vsearch_lca).")
```
...
is applied to identify features that are differentially abundant across sample groups.
ANCOM-BC has higher sensitivity than its successor ANCOM-BC2, but is also more prone to false positive results.
Comparisons between groups of samples is performed for specific metadata that can be found in folder
"))
...
<!-- Subsection on ANCOMBC2 results -->

```{r, results='asis'}
any_ancombc2 <- !isFALSE(params$ancombc2) || !isFALSE(params$ancombc2_formula)
```

```{r, eval = !isFALSE(any_ancombc2), results='asis'}
cat(paste0("
## ANCOM-BC2

[Analysis of Composition of Microbiomes with Bias Correction 2 (ANCOM-BC2)](https://pubmed.ncbi.nlm.nih.gov/38158428/)
is applied to identify features that are differentially abundant across sample groups.
ANCOM-BC2 is more conservative than its predecessor ANCOM-BC and can have lower sensitivity.
Comparisons between groups of samples is performed for specific metadata that can be found in folder
"))

if ( !isFALSE(params$ancombc2) && !isFALSE(params$ancombc2_formula) ) {
    cat("[qiime2/ancombc2/](../qiime2/ancombc2/) and [qiime2/ancombc2_formula/](../qiime2/ancombc2_formula/)")
} else if ( !isFALSE(params$ancombc2) ) {
    cat("[qiime2/ancombc2/](../qiime2/ancombc2/)")
} else if ( !isFALSE(params$ancombc2_formula) ) {
    cat("[qiime2/ancombc2_formula/](../qiime2/ancombc2_formula/)")
}
cat(".")

cat(paste0("
Test results are in separate folders following the scheme `Category-{treatment}-{taxonomic level}`. Here are the volcano plots:
"))
```

```{r, eval = !isFALSE(params$ancombc2), out.width="25%", fig.show='hold', fig.align='default'}
knitr::include_graphics( sort(strsplit( params$ancombc2, ",")[[1]]) )
```

```{r, eval = !isFALSE(params$ancombc2_formula), out.width="25%", fig.show='hold', fig.align='default'}
knitr::include_graphics( sort(strsplit( params$ancombc2_formula, ",")[[1]]) )
```
...
but if you run HOMES_amplicon with a sample metadata table (`--metadata`) any fields corresponding to a field in the template will be used.
"))
```
...
Data was processed using HOMES_amplicon `r ampliseq_version` (doi: [10.5281/zenodo.1493841](https://zenodo.org/badge/latestdoi/150448201))
([Straub et al., 2020](https://doi.org/10.3389/fmicb.2020.550420)) of the HOMES collection of workflows
...
if (!isFALSE(params$vsearch_lca_taxonomy)) {
    methods_taxonomic_classification <- c(methods_taxonomic_classification,
        paste("VSEARCH usearch_global with --lcaout and a user provided reference database", sep=""))
}
...
if (!isFALSE(params$barplot) || !isFALSE(params$alpha_rarefaction) || !isFALSE(params$diversity_indices_beta) || !isFALSE(params$ancom) || !isFALSE(any_ancombc)) {
    ...
    if (!isFALSE(any_ancombc2)) {
        qiime_final <- c(qiime_final,"used to find differential abundant taxa with ANCOM-BC2 ([Lin and Peddada, 2024](https://pubmed.ncbi.nlm.nih.gov/38158428/))")
    }
...
cat("in folder [pipeline_info](../pipeline_info) file `homes_ampli_shortseq_software_mqc_versions.yml`.")
```
...
if (!isFALSE(params$vsearch_lca_taxonomy)) {
    cat("Taxonomic classification by VSEARCH LCA:\n\n",
        "- database: `", params$vsearch_lca_ref_tax_title, "`\n\n",
        "- files: `", params$vsearch_lca_ref_tax_file, "`\n\n",
        "- citation: `", params$vsearch_lca_ref_tax_citation, "`\n\n", sep = "")
}
...
The proposed short methods description can be found in [MultiQC's Methods Description](../multiqc/multiqc_report.html#HOMES-ampliseq-methods-description),
versions of software collected at runtime in [MultiQC's Software Versions](../multiqc/multiqc_report.html#software_versions),
and a summary of non-default parameter in [MultiQC's Workflow Summary](../multiqc/multiqc_report.html#HOMES-ampliseq-summary).
"))
```
...
including software versions collected at runtime in file `homes_ampli_shortseq_software_mqc_versions.yml` (can be viewed with a text editor),
...
Please cite the [pipeline publication](https://doi.org/10.3389/fmicb.2020.550420) and any software tools used by the pipeline (see [citations](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon#citations)) when you use any of the pipeline results in your study.
