# HOMES_amplicon: Usage

## :warning: Please read this documentation on the HOMES website: [https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/usage](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/usage)
...
Short-read mode is R-native by default. HOMES_amplicon keeps taxonomy in DADA2 `assignTaxonomy()` and keeps downstream ecological analysis in R rather than dispatching to QIIME2. This avoids the QIIME2 container/runtime overhead and keeps taxonomy, diversity, ordination, differential abundance and PERMANOVA in a single R reporting path.
...
> [!TIP]
> For [Reproducibility](#reproducibility), specify the version to run using `-r` (= release, e.g. 2.17.0, please use the most recent release). See the [HOMES_amplicon website documentation](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters) for more information about pipeline specific parameters.
...
<p align="center">
    <img src="images/homes_amplicon_shortread_overview.svg" alt="HOMES_amplicon workflow overview" width="80%">
</p>

Nanopore amplicon runs use the same entry point with `--platform nanopore`:
...
<p align="center">
    <img src="images/homes_amplicon_nanopore_overview.svg" alt="HOMES_amplicon Nanopore workflow overview" width="80%">
</p>

Both platform branches write HTML reports. Short-read mode writes `multiqc/multiqc_report.html` and `summary_report/summary_report.html`; Nanopore mode writes `nanopore/report/homes_nanopore_report.html`.
...
> [!WARNING]
> Do not use `-c <file>` to specify parameters as this will result in errors. Custom config files specified with `-c` must only be used for [tuning process resource specifications](https://www.nextflow.io/docs/latest/config.html#configuring-pipelines), other infrastructural tweaks (such as output directories), or module arguments (args).

The above pipeline run specified with a params file in yaml format:

```bash
nextflow run HOMES_amplicon -profile docker -params-file params.yaml
```
...
You can also generate such `YAML`/`JSON` files via [HOMES/launch](https://github.com/Harper19/HOMES).

### Input specifications

The input data can be passed to HOMES_amplicon in three possible ways using the parameters `--input`, `--input_fasta`, or `--input_folder`.
...
¹: As of Silva version 138 optimized for classification of Bacteria and Archaea, not suitable for Eukaryotes; ²[`--dada_taxonomy_rc`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters#dada_taxonomy_rc) is recommended; ³: de-replicated at 85%, only for testing purposes; ⁴: quality of results might vary
...
Parameter guidance is given in [HOMES_amplicon website parameter documentation](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters/#taxonomic-assignment). Citations are listed in [`CITATIONS.md`](CITATIONS.md).

> [!TIP]
> Taxonomic reference databases can be stored and shared locally with [`--ref_taxonomy_storage`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters/#ref_taxonomy_storage). That way, remote files will be downloaded only if they are not available in the storage directory.
...
Information about sequencing data via [`--input`](#sample-sheet-input), region primers length information via [`--multiregion`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters#multiregion), and a taxonomic database via [`--sidle_ref_taxonomy`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters#sidle_ref_taxonomy) or [`--sidle_ref_tax_custom`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters#sidle_ref_tax_custom) with [`--sidle_ref_seq_custom`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters#sidle_ref_seq_custom) is required.
...
See the [HOMES_amplicon parameter documentation](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters) for more information about the parameters.
...
The set is provided with a spreadsheet passed to the [`--pplace_sheet`](https://github.com/Harper19/HOMES/tree/main/workflows/HOMES_amplicon/parameters#pplace_sheet), see example below.
...
First, go to the [HOMES releases page](https://github.com/Harper19/HOMES/releases) and find the pipeline version to use. Then specify this when running the pipeline with `-r` (one hyphen). Of course, you can switch to another version by changing the value after the `-r` flag.
...
The pipeline also dynamically loads configurations from [https://github.com/HOMES/configs](https://github.com/HOMES/configs) when it runs, making multiple config profiles for various institutional clusters available at run time. For more information and to check if your system is supported, please see the [HOMES/configs documentation](https://github.com/HOMES/configs#documentation).
...
Specify the path to a specific config file (this is a core Nextflow command). See the [HOMES website documentation](https://www.nextflow.io/docs/latest/config.html) for more information.

## Custom configuration

### Resource requests

Whilst the default requirements set within the pipeline will hopefully work for most people and with most input data, you may find that you want to customise the compute resources that the pipeline requests. Each step in the pipeline has a default set of requirements for number of CPUs, memory and time. For most of the pipeline steps, if the job exits with any of the error codes specified [here](https://github.com/HOMES/rnaseq/blob/4c27ef5610c87db00c3c5a3eed10b1d161abf575/conf/base.config#L18) it will automatically be resubmitted with higher resources request (2 x original, then 3 x original). If it still fails after the third attempt then the pipeline execution is stopped.

To change the resource requests, please see the [max resources](https://www.nextflow.io/docs/latest/process.html) and [customise process resources](https://www.nextflow.io/docs/latest/process.html) section of the HOMES website.

### Custom Containers

In some cases, you may wish to change the container or conda environment used by a pipeline steps for a particular tool. By default, HOMES workflows use containers and software from the [biocontainers](https://biocontainers.pro/) or [bioconda](https://bioconda.github.io/) projects. However, in some cases the pipeline specified version maybe out of date.

To use a different container from the default container or conda environment specified in a pipeline, please see the [updating tool versions](https://www.nextflow.io/docs/latest/container.html) section of the HOMES website.

### Custom Tool Arguments

A pipeline might not always support every possible argument or option of a particular tool used in pipeline. Fortunately, HOMES workflows provide some freedom to users to insert additional parameters that the pipeline does not include by default.

To learn how to provide additional arguments to a particular tool of the pipeline, please see the [customising tool arguments](https://www.nextflow.io/docs/latest/process.html) section of the HOMES website.

### HOMES/configs

In most cases, you will only need to create a custom config as a one-off but if you and others within your organisation are likely to be running HOMES workflows regularly and need to use the same settings regularly it may be a good idea to request that your custom config file is uploaded to the `HOMES/configs` git repository. Before you do this please can you test that the config file works with your pipeline of choice using the `-c` parameter. You can then create a pull request to the `HOMES/configs` repository with the addition of your config file, associated documentation file (see examples in [`HOMES/configs/docs`](https://github.com/HOMES/configs/tree/master/docs)), and amending [`homes_custom.config`](https://github.com/HOMES/configs/blob/master/homes_custom.config) to include your custom profile.

See the main [Nextflow documentation](https://www.nextflow.io/docs/latest/config.html) for more information about creating your own configuration files.

If you have any questions or issues please send us a message on [Slack](https://github.com/Harper19/HOMES/issues) on the [`#configs` channel](https://github.com/Harper19/HOMES/issues).
