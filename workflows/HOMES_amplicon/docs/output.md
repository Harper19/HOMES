...
## Platform-Specific HTML Reports

HOMES_amplicon uses one entry point with platform-specific reporting.

Short-read mode (`--platform short`) writes:

- `multiqc/multiqc_report.html`: aggregate QC and software report, including FastQC, Cutadapt, DADA2-related logs and versions where available.
- `summary_report/summary_report.html`: standalone R Markdown report covering read QC, denoising, ASV tables, DADA2 taxonomy and R-native downstream summaries.

Nanopore mode (`--platform nanopore`) writes:

- `nanopore/report/homes_nanopore_report.html`: standalone HTML report covering read QC, taxonomy-derived top taxa, relative abundance and diversity.
- `nanopore/abundance/*.counts.<tax_level>.tsv`: taxon count table.
- `nanopore/abundance/*.relative_abundance.<tax_level>.tsv`: relative abundance table.
- `nanopore/abundance/*.diversity.<tax_level>.tsv`: richness, Shannon and Simpson diversity table.
...
Most of the fields in the template will not be populated by the export process, but if you run HOMES_amplicon with a sample metadata table (`--metadata`) any fields corresponding to a field in the template will be used.
...
- `pipeline_info/`
  ...
  - Reports generated by the pipeline: `pipeline_report.html`, `pipeline_report.txt` and `homes_ampli_shortseq_software_mqc_versions.yml`. The `pipeline_report*` files will only be present if the `--email` / `--email_on_fail` parameter's are used when running the pipeline.
